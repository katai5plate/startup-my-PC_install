...
2. PC初期化
3. powershell で `(iwr "https://kutt.it/h2apcdoc").Content > ./guide.txt` してこの文章をダウンロード
4. 設定を全部やる
5. インストールを全部やる
...
- Winget はたまに謎の失敗をするので、2 回 import したほうがいい。
...
WinGet も規約同意が必要なので Apple.iTunes を先に入れる（適当）。  
...
- その後は、
  - [scoop-work](https://github.com/katai5plate/startup-my-PC_install/blob/master/scoop-work-2025.txt)
  - [scoop-make](https://github.com/katai5plate/startup-my-PC_install/blob/master/scoop-make-2025.txt)
  - [winget-np](https://github.com/katai5plate/startup-my-PC_install/blob/master/winget-np-2025.json)
- を `%username%` に保存。

### 自動インストール

#### どちらか一方で実行（おそらく制作用から先にやると思う）

```
winget import winget-mp-2025.json
```
